...
Droid::Droid(T6Vec3 pos)
{
	setBasicProperties();
	basePos_ = pos;
	position_ = pos + DROID_START_POS_FIX;
	...
	*objectDefRect_ = Default::createRectFromBottomCenter(getPosition(), DROID_BOUND_SIZE);
	Object::loadObjectResource(DROID_BASE_RESOURCE_LINK.c_str(), DROID_BASE_RESOURCE_ROWS,
								DROID_BASE_RESOURCE_COLUMNS, RESOURCE_MS);

	fire_ = new T6_Sprite(DROID_FIRE_RESOURCE_LINK.c_str(), DROID_FIRE_RESOURCE_COLUMNS,
		DROID_FIRE_RESOURCE_ROWS);

	door_ = new T6_Sprite(DROID_DOOR_RESOURCE_LINK.c_str(), DROID_DOOR_RESOURCE_COLUMNS,
		DROID_DOOR_RESOURCE_ROWS);

	staticBase_ = new T6_Texture(DROID_STATIC_BASE.c_str());
	livingBase_ = new T6_Texture(DROID_LIVING_BASE.c_str());
}
...
void Droid::draw()
{
	...
	if (!isDead())
	{
		...
		(*sprites_)->drawImage(getPosition(), nullptr, SCALE_NO, TRANSLATE_NO, DrawCenter::BOTTOMCENTER, ANGLE_NO, fadeColor_);
...
void Droid::activateUpdate()
{
	...
	case DROID_PREPARING:
		...
		if (timer_ >= DROID_PREPARING_TIME)
		{
			...
			timer_ = 0;
			position_.x = ((rand() % (randomRegion_.right - randomRegion_.left)) + randomRegion_.left);
			position_.y = ((rand() % (randomRegion_.top - randomRegion_.bottom)) + randomRegion_.bottom);
...
void Droid::setBasicProperties()
{
	...
	heathPoints_ = DROID_LIFE_POINTS;
}
