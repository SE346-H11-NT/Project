...
private:
			Object*				baseObjectForUpdate_;
			ExplodeBrideStatus	currentBridgeStatus_;
			ExplodeBridgeOrder	order_;

			Tile**				bridgehead_LinkedTile_;
			DWORD				destroyTimeRemaining_;
	static	Pretties**			destroyedBridgePretties_;

private:
	ExplodeBridge(Object* baseObject, T6Vec3 position, T6Rect boundRange, ExplodeBridgeOrder order);
	void	detonate();
