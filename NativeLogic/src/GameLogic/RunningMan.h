...
#define RUNNING_MAN_SHOOT_POINT_DISTANCE_FROM_POSITION(scaleX)	T6Vec3(scaleX * 16.0F, 26.0F, 0.0f)
...
{
public:
	RunningMan(T6Vec3 position, Velocity velocity, int numOfBullet);
	~RunningMan();
...
private:
	RunningManStatus status_;
	T6Vec3 respawPosition_;
	DWORD respawnTime_;
	int bulletPool_;
	int shootedBullet_;
	DWORD shootTime_;
